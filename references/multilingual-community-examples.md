...
这些是原始且经过安全筛选的示例，提炼自公开的 Seedance 2.0 多语言实践。请将它们作为结构和词汇模式参考，而非直接复制社区提示词。

## 边界

仅在明确良性创作意图时，语言混合才能降低误报率。切勿使用其他语言来隐藏不安全内容、受保护身份复制、真人模仿、血腥暴力或规避平台规则。

安全的语言混合意味着：

- 保持参考标签字面一致：`[Image1]`、`[Video1]`、`[Audio1]`、`@图1`、`@视频1`。
- 对创作者跨社区常用的专业摄影机术语使用英文：`35mm lens`、`locked medium shot`、`slow dolly-in`、`no text`。
- 当界面或协作方有预期时，使用中文进行简洁的角色绑定：`@图1锁定主体身份`、`@视频1仅参考运镜`。
- 仅将目标对话语言用于台词部分。
- 使用本地语言约束是为了消除歧义，而非弱化不安全请求。
- 如果安全提示词被拦截，请重写场景，提供更明确的制作背景、版权归属、相关的年龄/同意声明、非直白/非血腥的措辞，以及更窄化的参考角色范围。

## 有效模式

| 模式 | 适用场景 | 作用原理 |
|---|---|---|
| 中文角色绑定 + 英文摄影术语 | R2V、I2V、FLF2V | 中文作为参考更紧凑；英文保持镜头/摄影术语的通用性。 |
| 本地语言台词 + 英文走位/调度 | 唇音同步场景 | 台词保持自然，同时镜头和构图清晰明确。 |
| 英文安全/约束模块 | 社区共享提示词 | `no text`、`no watermark`、`original character` 和 `authorized reference` 易于审核。 |
| 单语言提示词 + 双语词汇表备注 | 翻译工作 | 当用户需要精炼的最终提示词时，避免语法混杂。 |
| 双语误报修复 | 因模糊词汇被拦截的安全提示词 | 明确道具、动作节拍或服装为摆拍、非血腥且为原创。 |

## 避免事项

- 切勿将不安全身份、伤害或成人内容术语翻译为其他语言以绕过过滤。
- 切勿在单一提示词中混合多种风格体系：单次镜头中混合电影级写实、水彩、动漫、黏土动画和纪录片手持风格，通常会导致画面混乱失真。
- 切勿让语言混合掩盖参考角色。如果 `[Video1]` 控制镜头，请明确说明它仅控制镜头。
- 切勿将一种语言中的受保护 IP 名称与另一种语言中的“通用”措辞混合；请将概念重写为原创世界观。
- 切勿过度堆砌多语言约束。五条精确约束胜过二十条负面短语。
...
1. 移除所有语言中的受保护身份、品牌和复制场景。
2. 声明安全制作背景：摆拍、原创、已授权、非直白/非血腥、无真人。
3. 将每个参考范围缩小至单一角色。
4. 对话保留说话者语言，但摄影机指令和约束条件使用对操作者最清晰的语言。
5. 将提示词精简为一个可视节拍和一个摄影机运动。
6. 若仍被拦截，请更改创意表面/设定，而不仅是更换语言。

## 全球制作交接模式

当协作者不仅是在输入提示词，而是为导演、剪辑师、本地化团队或客户准备镜头时使用这些模式。

| 语言环境 | 制作安全结构 |
|---|---|
| 中英镜头列表 | `Shot ID + @图/@视频 role map + action endpoint + Camera in English + 后期备注: textless/localized copy in post` |
| 日文审阅备注 | `ショット目的 + 保持する要素 + 修正する動き + postで追加する字幕/コピー` |
| 韩文台词交接 | `대사 + locked framing + speaker tag + 자막/더빙은 후반 작업에서 처리` |
| 西文客户版本管理 | `versión 15s/9:16 + producto protegido + texto en post + subtítulos separados` |
| 俄文交付说明 | `роль референса + что не менять + textless plate + отдельные субтитры/озвучка` |
